...
PATCHES=(
	...
	"${FILESDIR}"/001-bcm43xx-Add-bcm43xx-3wire-variant.patch
	"${FILESDIR}"/002-bcm43xx-The-UART-speed-must-be-reset-after-the-firmw.patch
	"${FILESDIR}"/003-Increase-firmware-load-timeout-to-30s.patch
	"${FILESDIR}"/004-Move-the-43xx-firmware-into-lib-firmware.patch
	"${FILESDIR}"/005-hostname-Use-phone-class-for-handhelds.patch
	"${FILESDIR}"/fix-endianness.patch
	"${FILESDIR}"/musl.patch
	"${FILESDIR}"/test-mesh-crypto.patch
)
